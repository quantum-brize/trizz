...
$namevalue ='UID="",TID="",ResultIndex="'.$ResultIndex.'",apiType="tbo",ORG_CODE="'.$dep_code.'",ORG_NAME="'.$depcityy.'",DES_CODE="'.$arr_codee.'",DES_NAME="'.$arrcityy.'",DEP_DATE="'.$msdate2['0'].'",DEP_TIME="'.createTimeArrDept($msdate2['1']).'",ARRV_DATE="'.$msdate1['0'].'",ARRV_TIME="'.createTimeArrDept($msdate1['1']).'",FLIGHT_CODE="'.trim($airlinecode).'",FLIGHT_NAME="'.$airline.'",FLIGHT_NO="'.trim($fno).'",FARE_TYPE="'.$ResultFareType.'",SEAT="'.$NoOfSeatAvailable.'",STOP="'.$flight_segment.'",AMT="'.$PublishedFare.'",DISPLAY_AMT="'.$PublishedFare.'",DUR="'.$hours.'H '.$minutes.'M'.'",S_CODE="'.$S_CODE.'",CN_CODE="'.$CN_CODE.'",OI="",PCC="'.$FareBasisCode.'",TAX_BREAKUP="0",FLIGHT_INFO="'.$FLIGHT_INFO.'",NET_FARE="'.$netamount.'",refundyes="'.$refstatuss.'",AirlineRemark="",F_CLASS="'.$PC.'",SECTOR="'.$flightType.'",adfare="'.$adfare.'",agfare="'.$agfare.'",clfare="'.$clfare.'",CON_DETAILS="",PARAM_DATA="'.addslashes(serialize($valArray)).'",agentId="'.$_SESSION['agentUserid'].'",searchJson="",tripType=2,roundTripFlight=0,acom="'.$commissiondeff.'",IsLCC="'.$IsLCC.'",netFareBeforecomm="'.($getCalCost2[1]-$commissiondeff).'",agentMarkup="'.$getAgentTaxonly[0].'",adminMarkup="'.$adminMarkup.'",totalTax="'.$totaldisplayTax.'",uniqueSessionId="'.$_SESSION['uniqueSessionId'].'"';
