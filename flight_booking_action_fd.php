...
$abpnr=GetPageRecord('*','pnr_details',' id="'.$res['FIXEDID'].'"');
$fixedpnr=mysqli_fetch_array($abpnr);
...
$totalPax=($adultmain+$childmain+$infantmain);
...
// echo "<pre>";
 ...
 if(trim($fixedpnr['pnr'])!='' && offlineflightifagentoffline($_SESSION['agentUserid'],$finalFlightname,$finalFareTypename)==0 && $onewayflightoffline==0){
 $pnr=$fixedpnr['pnr'];
 $bookingNumber='FD-'.rand(11111111,99999999);
 $status=2;
 $totalseat=($fixedpnr['block']+$totalPax);
	
	 
	//updatelisting('pnr_details','block="'.$totalseat.'"','id="'.$fixedpnr['id'].'" ');  
...
addlistinggetlastid('sys_balanceSheet',$a); 
	...
	if(offlineflightifagentoffline($_SESSION['agentUserid'],$finalFlightname,$finalFareTypename)==0 && $fixedpnr['addBy']==1){
