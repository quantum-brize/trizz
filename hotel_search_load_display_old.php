...
$finalCountadult= $_GET['noadults'.$n];
$adultJson.='{"PaxType":"AD"},';

}
$kkadult=$finalCountadult+$kkadult;
...
$finalCountchild= $_GET['nochilds'.$n];
...
$kkchild=$finalCountchild+$kkchild;
 ...
 $minprice=0;
	...
	//print_r($json_arr['HotelSearchResult']);
		  ...
		  $citydestination = $destination[0];
		    ...
		    $a=GetPageRecord('*','wig_hotel_json_bkp',' agentId="'.$_SESSION['agentUserid'].'" and uniqueSessionId="'.$_SESSION['uniqueHotelSessionId'].'" limit 0,10'); 
$res=mysqli_fetch_array($a);
		  
		 $val=(array)unserialize(stripslashes($res['searchJson']));
		 
		 
		
		 foreach($val as $hotelList){
		   if($n<20){
		  $count++;
...
<?php   
			...
			$a=GetPageRecord('*','hotelMaster',' agentId in(0,'.$agentid.') and hotelPhoto!="" and cityName="'.$citydestination.'" and status=1 order by id desc'); 
			...
			$rs=GetPageRecord('*','sys_HotelRoomTypeCost',' hotelId="'.$rest['id'].'" and maxPax<3 and rooms>0 and validFrom<="'.date('Y-m-d', strtotime($checkInDate)).'" and validTo>="'.date('Y-m-d', strtotime($checkOutDate)).'" order by adultCost asc');
			$hotelPrice=mysqli_fetch_array($rs); 
			...
			$ifrromremain=mysqli_fetch_array($rsroomre); 
				...
				if($adult<3 && $hotelPrice['id']>0 && $hotelPrice['maxPax']>$ifrromremain['remainroom']){
			 ...
			 $finalroomcost+=($adultcost+$childcostone+$childcosttwo)*$numberOfNights;
				...
				$rs=GetPageRecord('*','sys_HotelRoomTypeCost',' hotelId="'.$rest['id'].'" and maxPax>2 and rooms>0 and validFrom<="'.date('Y-m-d', strtotime($checkInDate)).'" and validTo>="'.date('Y-m-d', strtotime($checkOutDate)).'" order by adultCost asc');
			$hotelPrice=mysqli_fetch_array($rs); 
			...
			if($hotelPrice['id']!='' && $hotelPrice['maxPax']>$ifrromremain['remainroom']){
				...
				$childcosttwo=$hotelPrice['infantCost'];
				 ...
				 $rs=GetPageRecord('*','sys_HotelRoomTypeCost',' hotelId="'.$rest['id'].'" and rooms>0 and maxPax<3 and validFrom<="'.date('Y-m-d', strtotime($checkInDate)).'" and validTo>="'.date('Y-m-d', strtotime($checkOutDate)).'" order by adultCost asc');
			$hotelPrice=mysqli_fetch_array($rs); 
			...
			$ifrromremain=mysqli_fetch_array($rsroomre); 
				 ...
				 if($hotelPrice['maxPax']>$ifrromremain['remainroom']){
                  ...
                  <div class="col-lg-9">
                    ...
                    <div class="hotelbooking">
                        ...
                        <div class="hoteltext">
                            ...
                            <div class="reviewsection">
                                <p class="threeblue"><?php echo str_replace(' ','-',strtolower($rest['hotelType'])); ?></p> <span class="starcatht"><?php for($i=1; $i<=$rest['category']; $i++){ ?>
						 <i class="fa fa-star" aria-hidden="true"></i>
						   <?php } ?></span> 
                            </div>
                            <p class="relocation"><i class="fa fa-map-marker" aria-hidden="true"></i>
...
<?php } $amn++; } ?>
                  ...
                  <div class="col-lg-3">
                    <div class="bookbtn">
                      <h4>&#8377;<?php  echo round($hotelCost[2]+$totalgst); $allprice.=$hotelCost[2]+$totalgst.','; ?></h4>
                      <div class="blackbox">
		  ...
		  <input type="hidden" name="HotelSearchDetails" value="<?php echo htmlentities($hotelBasicJson); ?>" />
		   ...
		   $actual_link = str_replace('hotel_search_load.php','hotel-view?',$actual_link);
		   
		   $actual_link = $actual_link.'&hotelSearchId='.encode($rest['id']).'&hotelcost='.round($hotelCost[2]).'&ad='.$adultCount.'&cd='.$kkchild;
		   
		   ?>asdf
		   <a href="<?php echo $actual_link; ?>" target="_blank"><button type="button" class="btn btn-danger" style="width:100%;">View Room</button></a>
